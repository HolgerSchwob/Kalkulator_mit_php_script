...
let openEditorCb;
let inquiryStateRef;
let addVariantButton; // Reference to the "Add" button (only one, below list)
...
export function updateVariantsUI(variantsWithPrices, bookBlockState, personalizations) {
    ...
    variantsWithPrices.forEach(variant => {
        ...
        const deleteButtonHTML = canBeDeleted ? `<button class="button-danger remove-item-btn icon-btn" aria-label="Löschen" title="Löschen"><i data-lucide="trash-2"></i></button>` : '';
        ...
        header.innerHTML = `
            ...
            <div class="accordion-controls">
                ...
                <button class="button-secondary collapse-item-btn" style="display:${variant.isExpanded ? 'inline-flex' : 'none'}"><i data-lucide="chevron-up"></i> Einklappen</button>
                ${deleteButtonHTML}
            </div>
        `;
        ...
        if (variant.isExpanded) {
            ...
            let bindingSelectHTML = '<div class="select-field"><label>Bindungsart:</label><select class="binding-type-select">';
            CALC_CONFIG.bindings.forEach(b => {
                bindingSelectHTML += `<option value="${escapeAttr(b.id)}" ${b.id === variant.bindingTypeId ? 'selected' : ''}>${escapeHtml(b.name)}</option>`;
            });
            bindingSelectHTML += '</select></div>';

            let quantityInputHTML = `
                <div class="number-input-wrapper">
                    <label for="variant_quantity_${escapeAttr(variant.id)}">Anzahl:</label>
                    <div class="input-group">
                        <button type="button" class="btn-number" data-type="minus" data-field="variant_quantity_${escapeAttr(variant.id)}" aria-label="Menge verringern">-</button>
                        <input type="number" id="variant_quantity_${escapeAttr(variant.id)}" class="variant-quantity-input" min="1" value="${escapeAttr(String(variant.quantity))}">
                        <button type="button" class="btn-number" data-type="plus" data-field="variant_quantity_${escapeAttr(variant.id)}" aria-label="Menge erhöhen">+</button>
                    </div>
                </div>`;
            ...
            if (bindingConfig.options && bindingConfig.options.length > 0) {
                bindingConfig.options.forEach(optGroup => {
                    ...
                    optGroup.choices.forEach(choice => {
                        ...
                        optionsHTML += `<div><label><input type="radio" name="variant_option_${escapeAttr(optGroup.optionKey)}_${escapeAttr(variant.id)}" data-option-key="${escapeAttr(optGroup.optionKey)}" value="${escapeAttr(choice.id)}" ${isChecked ? 'checked' : ''}> ${escapeHtml(choice.name)}</label></div>`;
                    });
    ...
    // --- Show/hide the "Add Variant" button ---
 ...
 * @param {HTMLElement} addBtn - The button to add a new variant (below the list).
 */
